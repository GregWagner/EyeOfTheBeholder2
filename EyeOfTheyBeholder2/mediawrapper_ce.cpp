...
public:
	...
	//Bild laden
	void loadImage(int nr, char path[128]);
	...
	//Format80 Decoder
	int format80decode(unsigned char in[], unsigned char out[]);
...
void MEDIAWrapper::setupWindow(int posX, int posY, int width, int height, bool fullscreen)
{
	...
	//SDL screen erzeugen
	if(fullscreen)
		screen = SDL_SetVideoMode(320, 240, 16, SDL_FULLSCREEN); //z.B. SDL_FULLSCREEN
	else
		screen = SDL_SetVideoMode(320, 240, 16, 0); //z.B. SDL_FULLSCREEN
	...
	//Fonts
	char realpath[128];
	sprintf(realpath, "\\eob2\\data\\font1.bmp");
	font[0] = SFont_InitFont(SDL_LoadBMP(realpath), 0, 0, 0); 
	font[1] = SFont_InitFont(SDL_LoadBMP(realpath), 0, 0, 255); 
	font[2] = SFont_InitFont(SDL_LoadBMP(realpath), 254, 254, 254); 
	font[3] = SFont_InitFont(SDL_LoadBMP(realpath), 116, 232, 252);
	sprintf(realpath, "\\eob2\\data\\font2.bmp");
	font[4] = SFont_InitBigFont(SDL_LoadBMP(realpath), 254, 254, 254);
	font[5] = SFont_InitBigFont(SDL_LoadBMP(realpath), 252, 84, 84);
	font[6] = SFont_InitBigFont(SDL_LoadBMP(realpath), 84, 252, 252);
	font[7] = SFont_InitBigFont(SDL_LoadBMP(realpath), 0, 0, 0);

	//Mauszeiger ausschalten
	SDL_ShowCursor(SDL_DISABLE);

	//Sound
    FSOUND_Init(24000, 8, 0);
...
void MEDIAWrapper::loadImage(int imageID, char *path)
{
	...
	sprintf(realpath, "\\eob2\\%s.bmp", path);
...
void MEDIAWrapper::loadCPS(int imageID, char *filename, char *palette, int posX=0, int posY=0, int width=320, int height=200, bool transp=false, bool sprite=false)
{
	...
	char realpath[128];
	sprintf(realpath, "\\eob2\\%s", palette);

	palfile = fopen(realpath, "rb");
	...
	sprintf(realpath, "\\eob2\\%s", filename);
		...
		if (~code & 0x80) {
			...
			if (~code & 0x40) {
				...
				if (count < 0x3e) {
					...
					copyp = (__unaligned unsigned char*) &image_out[*(__unaligned unsigned short*)readp];
					...
					count = *(__unaligned unsigned short*)readp;
					readp += 2;
					code = *readp++;
...
void MEDIAWrapper::drawImage(int imageID, int posX, int posY, int fromPosX, int fromPosY, int width, int height)
{
	...
	if(width > 0 && height > 0)
	{
		if(imageID >= 0 && images[imageID])
		{
			...
			SDL_BlitSurface ( images[imageID], &rcSrc, screen, &rcDest );
		}
	}
	else
	{
		if(imageID >= 0 && images[imageID])
		{
			//weiss transparent setzen
			SDL_SetColorKey(images[imageID], SDL_SRCCOLORKEY, SDL_MapRGB(images[imageID]->format, 255, 255, 255));
			SDL_BlitSurface ( images[imageID], NULL, screen, &rcDest );
		}
	}
}
...
void MEDIAWrapper::copyImage(int imageFromID, int imageToID, int fromPosX, int fromPosY, int toPosX, int toPosY, int width, int height, int transpR=-1, int transpG=-1, int transpB=-1)
{
	...
	if(images[imageFromID] != NULL && transpR != -1 && transpG != -1 && transpB != -1)
...
void MEDIAWrapper::drawText(int fontID, int posX, int posY, int r, int g, int b, char* text, bool center)
{
	if(fontID < 4)
	{
		posY += 1; 
		if(center)
			posX = (this->screen->w / 2) - (SFont_TextWidth(font[0],text) / 2);
		if(r==0 && g==0 && b==0)
			SFont_Write (screen, font[0], posX, posY,text); 
		if(r==0 && g==0 && b==255)
			SFont_Write (screen, font[1], posX, posY,text); 
		if(r==255 && g==255 && b==255)
			SFont_Write (screen, font[2], posX, posY,text); 
		if(r==116 && g==232 && b==252)
			SFont_Write (screen, font[3], posX, posY,text); 
	}
	else
	{
		//schwarzen Schatten zeichnen
		SFont_BigWrite (screen, font[7], posX-1, posY+1,text);

		//Font zeichnen
		SFont_BigWrite (screen, font[fontID], posX, posY,text); 
	}
}
...
{
	char realpath[128];
	sprintf(realpath, "\\eob2\\%s.ogg", path);
	sound[nr] = FSOUND_Stream_Open(realpath, FSOUND_LOOP_OFF, 0, 0);
	if(sound[nr] == NULL)
		this->quit();
}
...
//
// gefülltest Viereck zeichnen, color ist rgba hex wert: 0x00000000
//
...
void MEDIAWrapper::fillRect(int posX, int posY, int width, int height, int r, int g, int b, int imageID = -1)
{
	...
	if((imageID != -1 && images[imageID] != NULL) || (imageID == -1))
		SDL_FillRect(imageID==-1?screen:images[imageID], &dstrect, SDL_MapRGB(imageID==-1?screen->format:images[imageID]->format, r, g, b));
}
...
{
	SDL_Rect dstrect = {posX, posY, 1, 1};
	SDL_FillRect(imageID==-1?screen:images[imageID], &dstrect, SDL_MapRGB(imageID==-1?screen->format:images[imageID]->format, current_colors[palOffset].r, current_colors[palOffset].g, current_colors[palOffset].b));
}
